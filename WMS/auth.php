...
function send_otp(string $email, string $name): string {
    ...
    db()->prepare('DELETE FROM wms_otp_codes WHERE email = ?')->execute([$email]);
    db()->prepare('INSERT INTO wms_otp_codes (email, code, expires_at) VALUES (?, ?, NOW() + INTERVAL 15 MINUTE)')
       ->execute([$email, $code]);

    $subject = '[Newton WMS] Your OTP Verification Code';
...
function verify_otp(string $email, string $code): bool {
    ...
    db()->prepare('UPDATE wms_otp_codes SET used = 1 WHERE id = ?')
       ->execute([$row['id']]);
    return true;
}
